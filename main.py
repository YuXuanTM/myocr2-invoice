...
import base64
import time
from tool.public_info import text_ocr_type
...
# 多行识别的
det_true_list = {"password_area", "buyer_address_telephone",
                 "buyer_bank_account", "seller_address_telephone",
                 "seller_bank_account", "remark",
                 "competent_tax_authorities_and_code"}
...
def ocr_and_set_det_true_value(ocr_result, det_img, det_label):
    if det_img:
        rr = predict(det_img)
        if rr:
            for index, line in enumerate(rr):
